...
//reset fields
const afterSubmit = (values, dispatch) => dispatch(reset("DetailsUpdate"));

//form logic
let FormScreenUpdate = (props) => {
  ...
  return (
    <div>
      <center>
        <h1 className="display-3 p-1">Update User Details</h1>
      </center>
...
const mapStateToProps = (state) => {
  if(state.users.user[0]){
  return {
    initialValues: {
      fullName: state.users.user[0].fullName||"",
      gender: state.users.user[0].gender||"",
      email: state.users.user[0].email||"",
      phone: state.users.user[0].phone||"",
      pincode: state.users.user[0].pincode||"",
    },
  };
}
};
